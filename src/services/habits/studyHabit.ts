...
// ─── Constants ─────────────────────────────────────────────────────────────
const TIMER_STATE_KEY = "@studyHabit_timerState";
...
// ─── In-memory state ───────────────────────────────────────────────────────
// The service manages a lightweight in-memory timer so the screen can query
// elapsed time without relying on React state for persistence.
...
export const StudyHabitService = {
  ...
  isComplete(goalMinutes: number): boolean {
    return StudyHabitService.getElapsedSeconds() >= goalMinutes * 60;
  },
  ...
  async saveTimerState(): Promise<void> {
    try {
      ...
      console.error("[studyHabit] saveTimerState error:", error);
    }
  },
   ...
   *
   * If the restored state is already past the goal, returns the state with
   * elapsed clamped to the goal so the screen can show completion.
   */
  async loadTimerState(): Promise<TimerState | null> {
    try {
      ...
      console.error("[studyHabit] loadTimerState error:", error);
      return null;
    }
  },
  ...
  async clearTimerState(): Promise<void> {
    ...
    try {
      ...
      console.error("[studyHabit] clearTimerState error:", error);
    }
  },
};
